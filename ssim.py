...
def ssim(img1, img2, window_size=11, window=None, size_average=True, full=False, val_range=None):
    """Calculates the SSIM between two 4D tensors."""
    # This function remains unchanged as it correctly processes 4D tensors.
...
def msssim(img1, img2, window_size=11, size_average=True, val_range=None, normalize=False):
    """
    MODIFIED to handle 5D inputs of shape (B, N, C, H, W).
    """
    # Check the number of dimensions of the input tensor.
    if img1.dim() == 5:
        # If the input is 5D, get its dimensions.
        B, N, C, H, W = img1.shape
        # Reshape the tensors to (B*N, C, H, W) to be processed by the ssim function.
        img1 = img1.view(B * N, C, H, W)
        img2 = img2.view(B * N, C, H, W)
    ...
    # The output will be a single scalar value, the mean MS-SSIM over all B*N images.
    return output
